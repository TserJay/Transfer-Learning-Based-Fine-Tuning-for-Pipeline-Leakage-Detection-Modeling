...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 2, num_classes=1000):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(3, 64, kernel_size=7 , stride=2, padding=4,bias=False),  #self.conv1 = nn.Conv1d(3, 64, kernel_size=7 , stride=2, padding=3,              
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True)
            )
        
        self.conv2 = nn.Sequential(
            nn.Conv1d(3, 32, kernel_size=7 , stride=2, padding=4, bias=False),
            nn.BatchNorm1d(32),
            nn.ReLU(inplace=True)
        )
        ...
        self.BiLSTM4 = nn.LSTM(input_size=112,
                               hidden_size=64,
                               num_layers=1,
                               batch_first=True,
                               bidirectional=True,
        ...
        self.con2= nn.Conv1d(in_channels=64, out_channels=256, kernel_size=1)
        self.pool2 = nn.AdaptiveAvgPool1d(896)
        ...
        self.con3 = nn.Conv1d(in_channels=64, out_channels=512, kernel_size=1)
        self.pool3 = nn.AdaptiveAvgPool1d(448)
        ...
        self.con4 = nn.Conv1d(in_channels=64, out_channels=256, kernel_size=1)
        self.pool4 = nn.AdaptiveAvgPool1d(224)
        ...
        #self.lora_fc_4 = nn.Linear(64,256)
        self.pool5 = nn.AdaptiveAvgPool1d(128)
    ...
    def forward(self, x, adapter_name):
        ...
        # print(x.shape)
        y = x
        #print(y.shape)   #  torch.Size([32, 3, 1792])
        x = self.conv1(x)

        #print(x.shape)
        z = self.conv1(y)  #  plus with target_lora funt    ([32, 64, 897])


        #print(z.shape)    # torch.Size([32, 64, 897])
...
################融合点2
        ...
        #print(x.shape)   # torch.Size([32, 256, 896])
        x = x + self.con2(self.pool2(z))
...
################融合点3
        #print(x.shape) # torch.Size([32, 512, 448])

        x = x + self.pool3(self.con3(z))
...
################融合点4
        #print(x.shape) # torch.Size([32, 256, 224])

        x = x + self.pool4(self.con4(z))



        # x = x + self.lora_fc_4(self.LoraLayer_4(z,adapter_name).view(-1, 64)).view(32, 256, 224)
        x = self.layer4(x)
        x, _ = self.BiLSTM4(x.permute(1, 0, 2))

################融合点5
        #print(x.shape) #torch.Size([32, 32, 128])


        y = self.conv2(y)
        #print(y.shape) # torch.Size([32, 32, 897]) 

        # y = self.LoraLayer_5(y, adapter_name)
        x = x + self.pool5(y)

        return x


class mnet_cnn(nn.Module): 
    ...
    def forward(self, x ):
        ...
        x = self.backbone(x,'adapter1')  #[32, 128, 113]  源域分支
...
#         # print(x.shape)
        ...
        x = self.ap(x.permute(1, 2, 0)).squeeze(-1)  #注意：它和nn.Linear一样，如果你输入了一个三维的数据，他只会对最后一维的数据进行处理
...
# ###############################################################################
        ...
        view = self.projetion_pos_1(x)  #孔径位置信息
        pos = self.fc1(view)
