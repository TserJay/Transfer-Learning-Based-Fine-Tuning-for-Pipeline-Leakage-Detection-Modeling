...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 2, num_classes=1000):
        ...
        self.con1 = nn.Sequential(
            ...
            nn.Conv1d(3, 64, kernel_size=7 , stride=2, padding=4,  #self.conv1 = nn.Conv1d(3, 64, kernel_size=7 , stride=2, padding=3,  
                               bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),

            nn.Conv1d(64, 128, kernel_size=3, stride=2, padding=3 // 2, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),

            nn.Conv1d(128, 64, kernel_size=3, stride=1, padding=3 // 2, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
        ...
        self.BiLSTM1 = nn.LSTM(input_size=449,
                               hidden_size=224,
                               num_layers=1,
                               batch_first=True,
                               bidirectional=True,
        ...
        self.layer4 = self._make_layer(block, 32, layers[1], stride=2)
        self.BiLSTM4 = nn.LSTM(input_size=224,
                               hidden_size=112,
                               num_layers=1,
                               batch_first=True,
                               bidirectional=True,
        ...
        self.LoraLayer_1 = LoraLayer(449,449)
        self.LoraLayer_1.update_layer('adapter1', r=4, lora_alpha=0.1, lora_dropout=0.2, init_lora_weights=True)
       

        # self.LoraLayer_2 = LoraLayer(897,896)
        # self.LoraLayer_2.update_layer('adapter1', r=4, lora_alpha=0.1, lora_dropout=0.2, init_lora_weights=True)
        # self.lora_fc_2 = nn.Linear(64,256)

        # self.LoraLayer_3 = LoraLayer(897,448)
        # self.LoraLayer_3.update_layer('adapter1', r=4, lora_alpha=0.1, lora_dropout=0.2, init_lora_weights=True)
        # self.lora_fc_3 = nn.Linear(64,512)

        self.LoraLayer_4 = LoraLayer(449,448)
        self.LoraLayer_4.update_layer('adapter1', r=4, lora_alpha=0.1, lora_dropout=0.2, init_lora_weights=True)
        self.lora_fc_4 = nn.Linear(64,256)

        self.LoraLayer_5 = LoraLayer(449,224)
        self.LoraLayer_5.update_layer('adapter1', r=4, lora_alpha=0.1, lora_dropout=0.2, init_lora_weights=True)
        self.lora_fc_5 = nn.Linear(64,128)
    ...
    def forward(self, x, adapter_name):
        ...
        x = self.con1(x)#  plus with target_lora funt    ([32, 64, 897])        print(x.shape)#  plus with target_lora funt    ([32, 64, 897])
        z = self.con1(y)

        #print(x.shape) # torch.Size([32, 64, 449])
...
################融合点1
        ...
        x = x + self.LoraLayer_1(z,adapter_name) 
        x = self.layer1(x)
        x, _ = self.BiLSTM1(x) 
        #print(x.shape)  # torch.Size([32, 256, 448])

################融合点2
   
        
        # x = x + self.lora_fc_2(self.LoraLayer_2(z,adapter_name).view(-1, 64)).view(32, 256, 896)
        # x = self.layer2(x)
        # x, _ = self.BiLSTM2(x)
...
################融合点3
        ...
        # x = x + self.lora_fc_3(self.LoraLayer_3(z,adapter_name).view(-1, 64)).view(32, 512, 448)
        # x = self.layer3(x)
        # x, _ = self.BiLSTM3(x)

################融合点4

       # print(self.LoraLayer_4(z,adapter_name).shape)


       # print(self.lora_fc_4(self.LoraLayer_4(z,adapter_name).view(-1, 64)).view(32, 256, 448).shape)
     
        x = x + self.lora_fc_4(self.LoraLayer_4(z,adapter_name).view(-1, 64)).view(32, 256, 448)
        x = self.layer4(x)
        x,_ = self.BiLSTM4(x)
...
################################
        ...
        # print(x.shape)   # torch.Size([32, 128, 224])
        # print(self.LoraLayer_5(z, adapter_name).shape)
        # print(self.lora_fc_5(self.LoraLayer_5(z, adapter_name).view(-1,64)).view(32,128,224).shape)


        x= x + self.lora_fc_5(self.LoraLayer_5(z, adapter_name).view(-1,64)).view(32,128,224)
...
class LORA_Net_12345_simple(nn.Module): 
    def __init__(self,  in_channel=3, kernel_size=3, in_embd=64, d_model=32, in_head=8, num_block=1, dropout=0, d_ff=128, out_num=12):
        ...
        '''
        super(LORA_Net_12345_simple, self).__init__()
        
    
        self.backbone = Res2Net(Bottle2neck, [1, 1], baseWidth = 128, scale = 2)  # [3,4,23,3]
        ...
        self.projetion_pos_1 = MLP(input_dim=128, hidden_dim=128, output_dim=64, num_layers=1)
        # self.projetion_pos_2 = MLP(input_dim=128, hidden_dim=64, output_dim=12, num_layers=1)
        #self.projetion_cls = MLP(input_dim=112, hidden_dim=64, output_dim=4, num_layers=1)
...
#                
#         # print(x.shape)   
        x_1 = self.ap(x).squeeze(-1)  #注意：它和nn.Linear一样，如果你输入了一个三维的数据，他只会对最后一维的数据进行处理
...
if __name__ == '__main__':
    ...
    model = Res2Net(Bottle2neck, [1, 1], baseWidth =32, scale = 2).to(0)  # [3,4,23,3]
