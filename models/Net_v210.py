...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 2):
        ...
        self.BiLSTM4 = LayerNormLSTM(64,32)
        ...
        self.LoraLayer_2 = LoraLayer(897,897)
        ...
        self.LoraLayer_3 = LoraLayer(897,449)
        ...
        self.LoraLayer_4 = LoraLayer(897,225)
    ...
    def forward(self, x, adapter_name):
        batch_size = x.size(0)  # 动态获取当前 batch_size
        y = x

        # 融合 Conv+BN+ReLU
        x = self.fused_conv_bn_relu1(x)
        y = self.fused_conv_bn_relu1(y)
        ...
        # 融合点2
        lora_fc_out_2 = self.lora_fc_2(lora_outputs[1].reshape(-1, 64)).reshape(batch_size, 256, 897)
        x = x + lora_fc_out_2
        x = self.layer2(x)
        x = self.BiLSTM2(x)
        ...
        # 融合点3
        lora_fc_out_3 = self.lora_fc_3(lora_outputs[2].reshape(-1, 64)).reshape(batch_size, 512, 449)
        x = x + lora_fc_out_3
        x = self.layer3(x)
        x = self.BiLSTM3(x)

        # 融合点4
        lora_fc_out_4 = self.lora_fc_4(lora_outputs[3].reshape(-1, 64)).reshape(batch_size, 256, 225)
        x = x + lora_fc_out_4
        x = self.layer4(x)
        x = self.BiLSTM4(x)  

        # 输出 
        y = lora_outputs[4]  # 单独拿出来给 y  

        # print(x.shape)
        # print(y.shape)    

        # y = self.lora_fc_5(lora_outputs[4].reshape(-1, 64)).reshape(batch_size, 32, 224)

        return x, y   



class Net_v210(nn.Module): 
    def __init__(self,  in_channel=3, kernel_size=3, in_embd=64, d_model=32, in_head=8, num_block=1, dropout=0, d_ff=128, out_num=12):
        super(Net_v210, self).__init__()
        ...
        self.projetion_pos_1 = MLP(input_dim=113, hidden_dim=64, output_dim=32, num_layers=1)
    ...
    def forward(self, x ):
        ...
        # x = self.backbone1(x)
        x,y = self.backbone(x,'adapter1')  # torch.Size([64, 64, 113])
